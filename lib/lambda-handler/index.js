 ...
 */
const saveString = async (bucketName, fileName, content) => {
  await writeData(bucketName, fileName, content)
  return {
 ...
 * @returns {body: string, headers: Record<string, string>, statusCode: number}
 */
const getHtmlBody = async (bucketName, fileName, initialContent) => {
  const dynamicString = await readData(bucketName, fileName) ?? initialContent
...
exports.handler = async (event) => {
  // to decide the action to be executed
  const { httpMethod, queryStringParameters, body } = event

  // choose the appropriate action
  if (httpMethod === 'POST' && body) {
    // store data
    return await saveString(process.env.BUCKET_NAME, FILE_NAME, body)
  } else if (httpMethod === 'GET' && queryStringParameters !== null) {
    // store data
    return await saveString(process.env.BUCKET_NAME, FILE_NAME, queryStringParameters.string)
  } else if (httpMethod === 'GET') {
    // retrieve data
    return getHtmlBody(process.env.BUCKET_NAME, FILE_NAME, INITIAL_CONTENT)
  }
}
